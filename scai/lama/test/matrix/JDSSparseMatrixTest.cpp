/**
 * @file test/matrix/JDSSparseMatrixTest.cpp
 *
 * @license
 ...
 * @endlicense
 *
 * @brief Contains only test specific for the JDS Sparse matrix
...
BOOST_AUTO_TEST_CASE_TEMPLATE( defaultConstructorTest, ValueType, scai_arithmetic_test_types )
{
    ...
    BOOST_CHECK_EQUAL( Matrix::JDS, matrix.getFormat() );
    const JDSStorage<ValueType>& local = matrix.getLocalStorage();
    const JDSStorage<ValueType>& halo = matrix.getHaloStorage();
...
BOOST_AUTO_TEST_CASE( sizeConstructorTest )
{
    ...
    JDSSparseMatrix<ValueType> matrix( numRows, numCols );
...
BOOST_AUTO_TEST_CASE( distConstructorTest )
{
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            JDSSparseMatrix<ValueType> matrix( rowDist, colDist );
...
BOOST_AUTO_TEST_CASE( storageConstructorTest )
{
    ...
    JDSStorage<ValueType> globalStorage;
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            JDSSparseMatrix<ValueType> matrix( globalStorage );
            JDSSparseMatrix<ValueType> matrix1( matrix, rowDist, colDist );
            JDSSparseMatrix<ValueType> matrix2_tmp( matrix, rowDist, repColDist );
            JDSSparseMatrix<ValueType> matrix2( matrix2_tmp.getLocalStorage(), rowDist, colDist );

            const JDSStorage<ValueType>& localStorage1 = matrix1.getLocalStorage();
            const JDSStorage<ValueType>& localStorage2 = matrix2.getLocalStorage();
...
BOOST_AUTO_TEST_CASE( transposeConstructorTest )
{
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            JDSSparseMatrix<ValueType> matrix1( globalStorage );
            ...
            JDSSparseMatrix<ValueType> matrix2( matrix1, transposeFlag );
...
BOOST_AUTO_TEST_CASE( virtualConstructorTest )
{
    ...
    for ( size_t i = 0; i < dists.size(); ++i )
    {
        ...
        common::unique_ptr<JDSSparseMatrix<ValueType> > newMatrix( matrix.newMatrix() );
   
        const JDSStorage<ValueType>& newLocalStorage = newMatrix->getLocalStorage();

        BOOST_CHECK_EQUAL( 0, newLocalStorage.getNumRows() );
        BOOST_CHECK_EQUAL( 0, newLocalStorage.getNumColumns() );

        common::unique_ptr<JDSSparseMatrix<ValueType> > copyMatrix( matrix.copy() );

        const JDSStorage<ValueType>& localStorage = matrix.getLocalStorage();
        const JDSStorage<ValueType>& copyLocalStorage = copyMatrix->getLocalStorage();
...
BOOST_AUTO_TEST_CASE( expConstructorTest )
{
    ...
    for ( size_t i = 0; i < dists.size(); ++i )
    {
        ...
        JDSSparseMatrix<ValueType> matrix2( mult * matrix1 );
        JDSSparseMatrix<ValueType> matrix3( matrix2 - matrix1 );

        const JDSStorage<ValueType>& localStorage1 = matrix1.getLocalStorage();
        const JDSStorage<ValueType>& localStorage3 = matrix3.getLocalStorage();
