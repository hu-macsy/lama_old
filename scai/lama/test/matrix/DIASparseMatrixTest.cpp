/**
 * @file test/matrix/DIASparseMatrixTest.cpp
 *
 * @license
 ...
 * @endlicense
 *
 * @brief Contains only test specific for the DIA Sparse matrix
...
{
    DIASparseMatrix<ValueType> matrix;
    // check zero sizes
    ...
    BOOST_CHECK_EQUAL( Matrix::DIA, matrix.getFormat() );
    const DIAStorage<ValueType>& local = matrix.getLocalStorage();
    const DIAStorage<ValueType>& halo = matrix.getHaloStorage();
...
BOOST_AUTO_TEST_CASE( sizeConstructorTest )
{
    ...
    DIASparseMatrix<ValueType> matrix( numRows, numCols );
...
BOOST_AUTO_TEST_CASE( distConstructorTest )
{
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            DIASparseMatrix<ValueType> matrix( rowDist, colDist );
...
BOOST_AUTO_TEST_CASE( storageConstructorTest )
{
    ...
    DIAStorage<ValueType> globalStorage;
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            DIASparseMatrix<ValueType> matrix( globalStorage );
            DIASparseMatrix<ValueType> matrix1( matrix, rowDist, colDist );
            DIASparseMatrix<ValueType> matrix2_tmp( matrix, rowDist, repColDist );
            DIASparseMatrix<ValueType> matrix2( matrix2_tmp.getLocalStorage(), rowDist, colDist );

            const DIAStorage<ValueType>& localStorage1 = matrix1.getLocalStorage();
            const DIAStorage<ValueType>& localStorage2 = matrix2.getLocalStorage();
...
BOOST_AUTO_TEST_CASE( transposeConstructorTest )
{
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            DIASparseMatrix<ValueType> matrix1( globalStorage );
            ...
            DIASparseMatrix<ValueType> matrix2( matrix1, transposeFlag );
...
BOOST_AUTO_TEST_CASE( virtualConstructorTest )
{
    ...
    for ( size_t i = 0; i < dists.size(); ++i )
    {
        ...
        common::unique_ptr<DIASparseMatrix<ValueType> > newMatrix( matrix.newMatrix() );
   
        const DIAStorage<ValueType>& newLocalStorage = newMatrix->getLocalStorage();

        BOOST_CHECK_EQUAL( 0, newLocalStorage.getNumRows() );
        BOOST_CHECK_EQUAL( 0, newLocalStorage.getNumColumns() );

        common::unique_ptr<DIASparseMatrix<ValueType> > copyMatrix( matrix.copy() );

        const DIAStorage<ValueType>& localStorage = matrix.getLocalStorage();
        const DIAStorage<ValueType>& copyLocalStorage = copyMatrix->getLocalStorage();
...
BOOST_AUTO_TEST_CASE( expConstructorTest )
{
    ...
    for ( size_t i = 0; i < dists.size(); ++i )
    {
        ...
        DIASparseMatrix<ValueType> matrix2( mult * matrix1 );
        DIASparseMatrix<ValueType> matrix3( matrix2 - matrix1 );

        const DIAStorage<ValueType>& localStorage1 = matrix1.getLocalStorage();
        const DIAStorage<ValueType>& localStorage3 = matrix3.getLocalStorage();
