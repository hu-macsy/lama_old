/**
 * @file test/matrix/COOSparseMatrixTest.cpp
 *
 * @license
 ...
 * @endlicense
 *
 * @brief Contains only test specific for the COO Sparse matrix
...
BOOST_AUTO_TEST_CASE_TEMPLATE( defaultConstructorTest, ValueType, scai_arithmetic_test_types )
{
    ...
    BOOST_CHECK_EQUAL( Matrix::COO, matrix.getFormat() );
    const COOStorage<ValueType>& local = matrix.getLocalStorage();
    const COOStorage<ValueType>& halo = matrix.getHaloStorage();
...
BOOST_AUTO_TEST_CASE( sizeConstructorTest )
{
    ...
    COOSparseMatrix<ValueType> matrix( numRows, numCols );
...
BOOST_AUTO_TEST_CASE( distConstructorTest )
{
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            COOSparseMatrix<ValueType> matrix( rowDist, colDist );
...
BOOST_AUTO_TEST_CASE( storageConstructorTest )
{
    ...
    COOStorage<ValueType> globalStorage;
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            COOSparseMatrix<ValueType> matrix( globalStorage );
            COOSparseMatrix<ValueType> matrix1( matrix, rowDist, colDist );
            COOSparseMatrix<ValueType> matrix2_tmp( matrix, rowDist, repColDist );
            COOSparseMatrix<ValueType> matrix2( matrix2_tmp.getLocalStorage(), rowDist, colDist );

            const COOStorage<ValueType>& localStorage1 = matrix1.getLocalStorage();
            const COOStorage<ValueType>& localStorage2 = matrix2.getLocalStorage();
...
BOOST_AUTO_TEST_CASE( transposeConstructorTest )
{
    ...
    for ( size_t irow = 0; irow < rowDists.size(); ++irow )
    {
        for ( size_t icol = 0; icol < colDists.size(); ++icol )
        {
            ...
            COOSparseMatrix<ValueType> matrix1( globalStorage );
            ...
            COOSparseMatrix<ValueType> matrix2( matrix1, transposeFlag );
...
BOOST_AUTO_TEST_CASE( virtualConstructorTest )
{
    ...
    for ( size_t i = 0; i < dists.size(); ++i )
    {
        ...
        common::unique_ptr<COOSparseMatrix<ValueType> > newMatrix( matrix.newMatrix() );
   
        const COOStorage<ValueType>& newLocalStorage = newMatrix->getLocalStorage();

        BOOST_CHECK_EQUAL( 0, newLocalStorage.getNumRows() );
        BOOST_CHECK_EQUAL( 0, newLocalStorage.getNumColumns() );

        common::unique_ptr<COOSparseMatrix<ValueType> > copyMatrix( matrix.copy() );

        const COOStorage<ValueType>& localStorage = matrix.getLocalStorage();
        const COOStorage<ValueType>& copyLocalStorage = copyMatrix->getLocalStorage();
...
BOOST_AUTO_TEST_CASE( expConstructorTest )
{
    ...
    for ( size_t i = 0; i < dists.size(); ++i )
    {
        ...
        COOSparseMatrix<ValueType> matrix2( mult * matrix1 );
        COOSparseMatrix<ValueType> matrix3( matrix2 - matrix1 );

        const COOStorage<ValueType>& localStorage1 = matrix1.getLocalStorage();
        const COOStorage<ValueType>& localStorage3 = matrix3.getLocalStorage();
